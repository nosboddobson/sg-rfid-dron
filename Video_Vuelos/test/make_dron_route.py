...
def create_drone_route_video(json_path, image_path, output_video_path, csv_path):
    ...
    if not route:
        print("No se pudo crear una ruta válida desde el CSV")
        return
    
    print(f"Ruta creada desde CSV: {route}")
...
if __name__ == "__main__":
    ...
    image_path = 'Video_Vuelos/layout/layout_patio_mina3.png'
    csv_path = 'Video_Vuelos/Inventarios/37_Elementos_JDE.csv'
    output_video_path = '37_ruta_inventario_dron.mp4'
    
    for path, desc in [(json_path, "JSON de bounding boxes"), 
                        (image_path, "imagen del layout"),
                        (csv_path, "CSV de datos")]:
        if not os.path.exists(path):
            print(f"Error: No se encontró el archivo {desc} en {path}")
            exit(1)
            
    create_drone_route_video(json_path, image_path, output_video_path, csv_path)