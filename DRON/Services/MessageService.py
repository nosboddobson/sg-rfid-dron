...
import logging
import task_manager as tm # Importa el módulo de gestión de tareas
import Suscriber_Reader as sr # Importa Suscriber_Reader
...
def srv_printer(msg):
    ...
    try:
        ...
        #201 indica que se apreto boton de forzar envio de lecturas.
        elif respuesta.status_code == 201:
            flag = True
            logging.info(f"Mensaje enviado exitosamente: {msg}")
            logging.info(f"Se ha recibido un mensaje de Solicitud de Archivo de Lectura.")
            tm.restart_get_reads(sr.get_reads, sr.reset_first_reader_flag)
        else:
